...
4. **✅ IG INSIGHTS WORK RIGHT NOW (live-verified 2026-07-23).** Tested `GET /{ig-media}/insights?metric=reach` on the reconnected account `madaboutmarketingg`'s OWN Reel (`18033029540819908`): **HTTP 200, `reach: 2619`** ("Accounts reached"). IG basic (`like_count:6`) also 200. **So IG reach/impressions/saved are accessible TODAY with the current permissions — `instagram_manage_insights` is either NOT required for own-media insights, OR the admin reconnect already granted it. My earlier "IG broken, needs the scope" conclusion was WRONG** — it was based on stale pre-reconnect prod snapshots + the code requesting the deprecated `impressions` metric. The earlier failing IG test used a media id from a DIFFERENT account (not loadable by that token). **IG needs NO new permission for insights.**
...
**Net correction to the plan:**
- **IG: NO new permission needed** — insights work now. The code fix (request `views` not `impressions`) is already shipped and correct. `instagram_manage_insights` in the scope list is harmless (may still help/be required for some media types) but is NOT the blocker I claimed.
- **FB: NOT a permission problem for impressions/reach — those metrics were DELETED by Meta.** The code must stop requesting `post_impressions`/`post_impressions_unique` (they 400 the whole call) and use only the valid metric set. FB "reach"/"impressions" columns should render **`—` (unavailable)** — honestly, permanently, because Meta doesn't expose them for Page posts anymore. `read_insights` is still worth requesting for `post_clicks`/`post_video_views` values.
- **So the answer to "only two permissions remain?" is NO.** IG needs zero new permissions. FB's gap is a **deprecated-metric code bug**, not a permission — and even fixed, FB impressions/reach are gone at the platform level (only clicks/video-views/reactions remain).
