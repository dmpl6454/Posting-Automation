...
- **ROUND 2 (same day, PR #140) — `<img>` must NEVER receive a video URL.** A 1.6GB 4K camera file still killed the Safari tab at ~30% AFTER the churn fix. Empirical matrix (Playwright WebKit 26.0, real file): `<img src={videoBlob}>` = **+1.57GB RSS full-file ingest** (WebKit buffers the whole blob before giving up; Chromium sniffs + cancels in 9ms, so Chrome hides the bug); `<video preload="metadata">` = flat ~+180MB but with a ~1.2GB transient read burst on idle; the multipart XHR loop itself = **flat 316MB for all 209 parts** (fully exonerated). The IG/FB/X/LI/generic previews all fed `mediaUrls[0]` into bare `<img>` — Instagram is the DEFAULT preview tab. FIX: all preview media renders through **[PreviewMedia](apps/web/components/previews/preview-media.tsx)** (`mediaKinds` prop threaded from ComposeTab; extension sniff fallback): image→`<img>`, remote video→metadata `<video>`, **local blob video→static placeholder (NO media element)**. ComposeTab tile skips its inline `<video>` for local files > 256MB, and the aspect probe releases its element the MOMENT metadata arrives. Locked by [preview-media-classify.test.ts](apps/web/lib/preview-media-classify.test.ts). Do NOT add any `<img src={mediaUrls[...]}>` back into previews, and route any new media rendering through PreviewMedia.
  - **🔴 RECURRENCE via CLASSIFICATION, not rendering (2026-09-01).** ComposeTab kept a LOCAL, case-sensitive video test (`/\.(mp4|webm|mov)/`, no `/i`, missing m4v/ogv) for its media tiles + the preview `mediaKinds`, while two sibling classifiers in the SAME file were correct. A Media-Library pick or restored draft has **no `File` object**, and S3 keys preserve the original extension's CASE (`fileName.split(".").pop()` — iPhone videos are `.MOV`) → the video classified as IMAGE → broken `<img>` tile, the **Thumbnail and Super-text controls hidden** (both gate on `isVideo`), and `classifyMediaUrl` **trusted the caller's kind**, feeding the video into `<img>` — the exact WebKit memory kill this bullet documents, reached through a bad prop instead of a bad render site. Owner-reported as "our work has been sabotaged". **FIX:** ONE exported classifier — `isVideoMediaItem` + `VIDEO_EXT_RE` in [preview-media.tsx](apps/web/components/previews/preview-media.tsx) — shared by the tile, the submit gates (`firstVideoUrl`/`hasVideoAttached`) and `mediaKinds`; and `classifyMediaUrl` now OVERRIDES a caller's `kind:"image"` when the URL matches a video extension (fail toward `<video>` — misrendering an image is cosmetic, `<img>` on a video is an OOM). Do NOT reintroduce a local video classifier in ComposeTab, and never `trust kind` blindly again. Same batch: **Generate Carousel is hidden when a video is attached** (`!hasVideoAttached` — image slides + a video is not a publishable combination). Locked in [preview-media-classify.test.ts](apps/web/lib/preview-media-classify.test.ts) (one prior assertion there encoded the blind-trust behavior and was updated with a note). **VIDEO_EXT_RE is now the ONLY video-extension regex in the repo — verify with `grep -rn "mp4|webm|mov" apps/web` before adding another.**
  - **🔴 A SECOND, INDEPENDENT instance of the same hazard was found DURING verification of the above and shipped in a follow-up (PR #180).** [youtube-preview.tsx](apps/web/components/previews/youtube-preview.tsx) computed `isVideo = firstKind ? firstKind === "video" : isVideoUrl(url)` — trusting a caller's kind ABSOLUTELY — and its else-branch (the wrong-media-type warning) rendered a **bare `<img src={firstMedia}>`**, bypassing PreviewMedia entirely. So the same `.MOV` mislabel would have fed a video into `<img>` on the YouTube preview tab even after the tile was fixed. It now defers to `classifyMediaUrl` (widened for this tab only: `blob:` counts as video, since YouTube accepts nothing else and a blob carries no extension) and the warning branch renders through `PreviewMedia`. **LESSON: a safety rule enforced only at the render site is defeated by any upstream classifier that mislabels its input — and "route everything through PreviewMedia" was already documented, yet one bare `<img>` had survived. After fixing a classifier, grep for EVERY render site AND every other copy of the classifier; the fix is not done until both greps come back clean.**
...
`sendPublishReportEmail` in [post-publish.worker.ts](apps/worker/src/workers/post-publish.worker.ts) fires for EVERY publish path (compose/scheduled/autopilot/newsgrid/chat — single worker funnel). Redesigned: **recipient = the post CREATOR** (`post.createdById`; falls back to org OWNERs only for creatorless system posts — logged), outcome subjects (`✅/⚠️/❌ N/M channels`). **Body since 2026-09-15 (owner ask): the live post links ONLY, one per line** — no platform, channel or timestamp — so they can be copied by hand or read by a script; the plain-text part is the same URLs, one per line, with the dashboard link deliberately kept OUT of that block. A failed channel has no link, so a count line ("N channels failed to publish") and an "N published posts have no public link" line stop a partial failure reading as a shorter list. A channel parked as needs-check (`ambiguousAt` set, stored FAILED) is counted separately as "could not be confirmed and may already be live — check before retrying", **never as failed** (the 2026-08-18 duplicate-post lesson), and the heading says "may not have published" when nothing published and one is unconfirmed. A URL containing whitespace, a control character or a line separator is **not listed** — `safeHref` only checks the prefix and self-hosted WordPress/Mastodon return URLs verbatim, so one could forge extra link lines in the machine-read text part; it counts toward the "no public link" note. Both checks live in `buildPublishEmail`, NOT `safeHref`, precisely so the CSV cannot change. The per-channel detail (platform, channel, handle, url, status, UTC, IST) lives in the attached CSV, which is **unchanged and locked byte-for-byte** in publish-email.test.ts — do not let an email edit touch it. Template is PURE + tested in [publish-email.ts](apps/worker/src/lib/publish-email.ts) — **all user content HTML-escaped, hrefs must be http(s)** (the old inline template interpolated raw). Never let email failure break publish (try/catch preserved); console fallback when SMTP unset. SMTP already configured on the worker (BO-01).
