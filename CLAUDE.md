...
- **REP-4 — Canva-like free-drag positioning.** Optional `logoPosXY?: {xPct,yPct}` + `hookPosXY?: {xPct,yPct}` on `StaticCreativeOptions`. A `logoCssBody` helper returns the EXACT existing corner CSS when absent (byte-identical) and `position:absolute;left:X%;top:Y%;transform:translate(-50%,-50%)` when present; applies to the 4 `.logo`-CSS builders (premium_editorial, hook_bars, bold_typographic [its `display:flex` tail is preserved], carousel body/cta) — NOT tweet_card/postcard_grid (inline logo). `clampPct` (finite + 0–100) guards the numeric CSS; do NOT route xPct/yPct through the string sanitizers. The hook `.bar` gets an inline absolute style only when `hookPosXY` is set. UI: a live-preview drag layer in the **static single-image branch only** (video/carousel render no drag layer); coords default null, reset on each new generate, sent only when non-null. The existing `logoPosition` corner picker stays as the discrete fallback.
