...
function BulkScheduleSection() {
  ...
  const toggleAll = () => {
    ...
    if (selectedIds.size === data.posts.length) setSelectedIds(new Set());
    else setSelectedIds(new Set(data.posts.map((p: any) => p.id)));
  };

  const handleSchedule = () => {
    if (selectedIds.size === 0 || !scheduledAt) return;
    bulkSchedule.mutate({
      items: Array.from(selectedIds).map((postId) => ({
        postId,
                ...
                <TableRow>
                  <TableHead className="w-12">
                    <input
                      type="checkbox"
                      checked={
                        selectedIds.size === data.posts.length && data.posts.length > 0
                      }
                      onChange={toggleAll}
  ...
  const csvImport = trpc.bulk.csvImport.useMutation({
    onSuccess: (result) => {
      toast({
        ...
        description: `${result.imported} post(s) imported.${
  ...
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const scheduled =
          scheduledAtIdx !== -1 ? cols[scheduledAtIdx]?.trim() : undefined;
  ...
  const handleImport = () => {
    ...
    reader.onload = (event) => {
      ...
      csvImport.mutate({
        ...
        scheduledAt: globalScheduledAt
          ? new Date(globalScheduledAt).toISOString()
          : undefined,
      });
    };
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        >
          ...
          <p className="mt-1 text-xs text-muted-foreground">Accepts .csv files</p>
          <input
            ref={fileInputRef}
            type="file"
            accept=".csv"
        ...
        {parseErrors.length > 0 && (
          <div className="rounded-md border border-yellow-200 bg-yellow-50 p-3">
            <p className="mb-1 text-sm font-medium text-yellow-800">Warnings:</p>
