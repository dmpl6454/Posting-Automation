...
KEYWORDS: ${(brief.keywords || []).join(", ")}`;
      ...
      let renderedMimicryEngine: "gemini-img2img" | "openai-described" | "gemini-composite" | null = null;
          ...
          try {
            ...
            if (input.referenceMimicry && aestheticRefImage) {
              ...
              if (m && m.engine !== "template" && m.imageBase64) {
                ...
                renderedMimicryEngine = m.engine as "gemini-img2img" | "openai-described" | "gemini-composite";
...
Return ONLY the JSON array, no other text.`;
          ...
          for (let batchStart = 0; batchStart < allSlides.length; batchStart += BATCH_SIZE) {
            ...
            const batchPromises = allSlides.slice(batchStart, batchEnd).map(async (slideMeta, batchIdx) => {
              ...
              try {
                ...
                let slideMimicryEngine: "gemini-img2img" | "openai-described" | "gemini-composite" | null = null;
                if (isCover && input.referenceMimicry && aestheticRefImage) {
                  ...
                  if (m && m.engine !== "template" && m.imageBase64) {
                    ...
                    slideMimicryEngine = m.engine as "gemini-img2img" | "openai-described" | "gemini-composite";
                  } else {
  ...
  regenerateImage: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      let regenMimicryEngine: "gemini-img2img" | "openai-described" | "gemini-composite" | null = null;
      let creative: { imageBase64: string; mimeType: string; bgSource?: "ai" | "stock"; imageEngine?: "gemini" | "openai" };
      if (input.referenceMimicry && regenAestheticRef) {
        try {
          ...
          if (m.engine !== "template" && m.imageBase64) {
            ...
            regenMimicryEngine = m.engine as "gemini-img2img" | "openai-described" | "gemini-composite";
          } else {
